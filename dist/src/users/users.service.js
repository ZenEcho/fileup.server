...
let UsersService = class UsersService {
    ...
    constructor(prisma, mailerService, mailVerificationPolicyService) {
        ...
        this.emailVerificationService = new email_verification_service_1.EmailVerificationService(this.prisma, this.mailerService, this.mailVerificationPolicyService);
    }
    async findOrCreate(githubId, profile) {
    ...
    async findOrCreateByOAuth(payload) {
        const providerUserId = payload.providerUserId.trim();
        if (!providerUserId) {
            throw new common_1.BadRequestException('OAUTH_PROVIDER_ID_REQUIRED');
        }
        ...
        if (byIdentity) {
            if (!byIdentity.isActive) {
                throw new common_1.UnauthorizedException('USER_OAUTH_UNBOUND_REBIND_REQUIRED');
            }
            ...
            if (linked) {
                await this.syncOAuthUserByIdentity(byIdentity.userId, payload, email, providerUserId);
                return linked;
            }
        ...
        if (payload.provider === 'GITHUB') {
            ...
            if (byLegacyGithubId) {
                const existingByProvider = await this.prisma.userOAuthAccount.findUnique({
                    where: {
                        provider_userId: {
                            provider: payload.provider,
                ...
                if (existingByProvider &&
                    ...
                    throw new common_1.ConflictException('USER_GITHUB_ALREADY_BOUND');
                }
        ...
        if (email && payload.emailVerified) {
            ...
            if (byEmail) {
                ...
                if (existingByProvider &&
                    existingByProvider.providerUserId !== providerUserId) {
                    throw new common_1.ConflictException('USER_OAUTH_ALREADY_BOUND');
                }
                await this.prisma.$transaction(async (tx) => {
                    if (!existingByProvider) {
                        await tx.userOAuthAccount.create({
        ...
        const username = await this.buildGithubUsername(this.normalizeUsername(payload.username));
        const now = new Date();
    ...
    async bindOAuthAccount(userId, payload) {
        const providerUserId = payload.providerUserId.trim();
        if (!providerUserId) {
            throw new common_1.BadRequestException('OAUTH_PROVIDER_ID_REQUIRED');
        }
        ...
        if (!current.passwordHash) {
            throw new common_1.BadRequestException(payload.provider === 'GITHUB'
                ? 'USER_GITHUB_BIND_LOCAL_ONLY'
                : 'USER_GOOGLE_BIND_LOCAL_ONLY');
        }
        const existingOnCurrent = current.oauthAccounts.find((item) => item.provider === payload.provider);
        const canReactivateOnCurrent = Boolean(existingOnCurrent) &&
        ...
        if (existingOnCurrent && !canReactivateOnCurrent) {
            ...
            throw new common_1.ConflictException(payload.provider === 'GITHUB'
        ...
        let shouldReactivateOAuthIdentity = Boolean(byIdentity && byIdentity.userId === userId && !byIdentity.isActive);
        if (byIdentity && byIdentity.userId !== userId) {
            ...
            if (!resolveResult.resolved) {
                throw new common_1.ConflictException('USER_OAUTH_ALREADY_BOUND_TO_OTHER');
            }
        ...
        if (email) {
            ...
            if (existedByEmail && existedByEmail.id !== userId) {
                throw new common_1.ConflictException('OAUTH_EMAIL_CONFLICT');
            }
        }
        await this.prisma.$transaction(async (tx) => {
    ...
    async resolveOAuthIdentityConflictForBind(input) {
        ...
        if (source.role === prisma_client_1.Role.ADMIN ||
            source.passwordHash ||
    ...
    async bindGithubAccount(userId, payload) {
    ...
    async isOAuthProviderBound(userId, provider) {
    ...
    async unbindMyOAuthProvider(userId, provider) {
        await this.unbindOAuthProvider(userId, provider, {
            byAdmin: false,
        });
        return this.getMyProfile(userId);
    }
    async forceUnbindOAuthProviderByAdmin(operatorId, targetUserId, provider) {
        const result = await this.unbindOAuthProvider(targetUserId, provider, {
            operatorId,
            byAdmin: true,
        });
        await this.createAdminActionLog(operatorId, targetUserId, 'FORCE_UNBIND_OAUTH', result);
        return this.findAdminUserById(targetUserId);
    }
    async unbindOAuthProvider(userId, provider, options) {
        ...
        if (!authProvidersBefore.includes(targetProvider)) {
            throw new common_1.BadRequestException('USER_OAUTH_PROVIDER_NOT_BOUND');
        }
        if (authProvidersBefore.length <= 1) {
            throw new common_1.BadRequestException('USER_LOGIN_METHOD_LAST_ONE');
        }
        const activeIdentity = user.oauthAccounts.find((item) => item.provider === provider && item.isActive);
        const shouldClearGithubLegacy = provider === 'GITHUB' &&
        ...
        return {
            ...
            authProvidersBefore,
            authProvidersAfter: authProvidersBefore.filter((item) => item !== targetProvider),
        };
    }
    async syncOAuthUserByIdentity(userId, payload, email, providerUserId) {
        const current = await this.prisma.user.findUnique({
            where: {
                id: userId,
            },
    ...
    findLocalLoginCandidates(identifier, loginByEmail) {
        if (loginByEmail) {
    ...
    async requestMyEmailChange(userId, nextEmailInput) {
        const nextEmail = this.normalizeNullableEmail(nextEmailInput);
        this.validateEmail(nextEmail);
        if (!nextEmail) {
            throw new common_1.BadRequestException('USER_EMAIL_REQUIRED');
        }
        const user = await this.prisma.user.findUnique({
            where: {
                id: userId,
        ...
        if (user.email === nextEmail) {
            throw new common_1.BadRequestException('USER_EMAIL_SAME_AS_CURRENT');
        }
        ...
        await this.prisma.user.update({
            ...
            data: {
                ...
                pendingEmailPurpose: prisma_client_1.PendingEmailPurpose.EMAIL_CHANGE,
            },
        });
        return this.emailVerificationService.sendVerificationForUser({
            ...
            purpose: prisma_client_1.EmailVerificationPurpose.EMAIL_CHANGE,
        });
    }
    resendMyEmailChangeVerification(userId) {
        return this.emailVerificationService.resendForPendingPurpose({
            userId,
            purpose: prisma_client_1.EmailVerificationPurpose.EMAIL_CHANGE,
        });
    }
    async requestMyLocalBind(userId, payload) {
        const email = this.normalizeNullableEmail(payload.email);
        ...
        if (!email || !password || !confirmPassword) {
            throw new common_1.BadRequestException('USER_LOCAL_BIND_REQUIRED_FIELDS');
        }
        this.validateEmail(email);
        this.validatePassword(password);
        if (password !== confirmPassword) {
            throw new common_1.BadRequestException('USER_PASSWORD_CONFIRM_MISMATCH');
        }
        const user = await this.prisma.user.findUnique({
            where: {
                id: userId,
        ...
        const hasOAuthIdentity = Boolean(user.githubId) || user.oauthAccounts.length > 0;
        if (!hasOAuthIdentity) {
            throw new common_1.BadRequestException('USER_LOCAL_BIND_OAUTH_ONLY');
        }
        if (user.passwordHash) {
            throw new common_1.BadRequestException('USER_LOCAL_BIND_ALREADY_BOUND');
        }
        await this.ensureEmailAvailable(email, userId);
        const pendingPasswordHash = await this.passwordService.hashPassword(password);
        await this.prisma.user.update({
            where: {
                id: userId,
            },
            data: {
                pendingEmail: email,
                pendingEmailPurpose: prisma_client_1.PendingEmailPurpose.LOCAL_BIND,
                pendingPasswordHash,
            },
        });
        return this.emailVerificationService.sendVerificationForUser({
            ...
            email,
            username: user.username,
            purpose: prisma_client_1.EmailVerificationPurpose.LOCAL_BIND,
        });
    }
    resendMyLocalBindVerification(userId) {
        return this.emailVerificationService.resendForPendingPurpose({
            userId,
            purpose: prisma_client_1.EmailVerificationPurpose.LOCAL_BIND,
        });
    ...
    async changeMyPassword(userId, payload) {
        ...
        const confirmNewPassword = this.normalizePassword(payload.confirmNewPassword);
        const currentPassword = this.normalizePassword(payload.currentPassword);
        if (!newPassword || !confirmNewPassword) {
            throw new common_1.BadRequestException('USER_PASSWORD_REQUIRED');
        }
        if (newPassword !== confirmNewPassword) {
            throw new common_1.BadRequestException('USER_PASSWORD_CONFIRM_MISMATCH');
        }
        ...
        if (hasOAuthIdentity && !user.passwordHash) {
            throw new common_1.BadRequestException('USER_LOCAL_BIND_REQUIRED');
        }
        if (user.passwordHash) {
            if (!currentPassword) {
                throw new common_1.BadRequestException('USER_PASSWORD_CURRENT_REQUIRED');
            }
            const matched = await this.passwordService.verifyPassword(currentPassword, user.passwordHash);
            if (!matched) {
                throw new common_1.ForbiddenException('USER_PASSWORD_INCORRECT');
            }
            const sameAsOld = await this.passwordService.verifyPassword(newPassword, user.passwordHash);
            if (sameAsOld) {
                throw new common_1.BadRequestException('USER_PASSWORD_NOT_CHANGED');
            }
        }
    ...
    async resendMyVerification(userId) {
        const user = await this.prisma.user.findUnique({
            where: {
                id: userId,
            },
    ...
    async findAdminUsers(input) {
        ...
        const pageSize = Math.min(this.normalizePositiveInt(input.pageSize, 20), 100);
        ...
        return {
            items: rows.map((row) => this.mapAdminUser(row, mailVerificationEnforced)),
            total,
            page,
            pageSize,
        };
    ...
    async updateUserByAdmin(operatorId, targetUserId, payload) {
        const current = await this.prisma.user.findUnique({
            where: {
                id: targetUserId,
            },
        ...
        if (Object.keys(data).length > 0) {
            ...
            await this.createAdminActionLog(operatorId, targetUserId, 'UPDATE_PROFILE', {
                changedFields,
            });
        }
        return this.findAdminUserById(targetUserId);
    }
    async updateRoleByAdmin(operatorId, targetUserId, role) {
        if (role !== 'ADMIN' && role !== 'DEVELOPER') {
            throw new common_1.BadRequestException('USER_ROLE_INVALID');
        }
        if (operatorId === targetUserId) {
            throw new common_1.ForbiddenException('USER_SELF_ROLE_CHANGE_FORBIDDEN');
        }
        const current = await this.prisma.user.findUnique({
            where: {
                id: targetUserId,
    ...
    async updateStatusByAdmin(operatorId, targetUserId, status) {
        if (status !== 'ACTIVE' && status !== 'BANNED') {
            throw new common_1.BadRequestException('USER_STATUS_INVALID');
        }
        if (operatorId === targetUserId) {
            throw new common_1.ForbiddenException('USER_SELF_STATUS_CHANGE_FORBIDDEN');
        }
        const nextStatus = status === 'ACTIVE' ? prisma_client_1.UserStatus.ACTIVE : prisma_client_1.UserStatus.DISABLED;
        const current = await this.prisma.user.findUnique({
            where: {
                id: targetUserId,
            },
    ...
    async resendVerificationByAdmin(operatorId, targetUserId) {
        ...
        await this.createAdminActionLog(operatorId, targetUserId, 'RESEND_VERIFICATION', {
            result,
        });
        return result;
    }
    async resetPasswordByAdmin(operatorId, targetUserId, input) {
        if (input.mode !== 'LINK' && input.mode !== 'TEMP_PASSWORD') {
            throw new common_1.BadRequestException('USER_PASSWORD_RESET_MODE_INVALID');
        }
        const target = await this.prisma.user.findUnique({
            where: {
                id: targetUserId,
        ...
        if (input.mode === 'LINK') {
            ...
            const ttlMinutes = Number(process.env['PASSWORD_RESET_TOKEN_TTL_MINUTES'] ||
        ...
        await this.createAdminActionLog(operatorId, targetUserId, 'RESET_PASSWORD', {
            mode: input.mode,
            result,
        });
        return result;
    ...
    async confirmPasswordReset(payload) {
        ...
        if (!record || record.expiresAt.getTime() <= Date.now()) {
            throw new common_1.UnauthorizedException('PASSWORD_RESET_TOKEN_INVALID');
        }
        const nextPasswordHash = await this.passwordService.hashPassword(newPassword);
        const now = new Date();
    ...
    validateUsername(username) {
        if (!USERNAME_PATTERN.test(username)) {
            throw new common_1.BadRequestException('USER_USERNAME_INVALID');
        }
    }
    validateEmail(email) {
        if (!email) {
        ...
        if (!EMAIL_PATTERN.test(email)) {
            throw new common_1.BadRequestException('USER_EMAIL_INVALID');
        }
    }
    validatePassword(password) {
        if (password.length < MIN_PASSWORD_LENGTH ||
            password.length > MAX_PASSWORD_LENGTH) {
            throw new common_1.BadRequestException('USER_PASSWORD_WEAK');
        }
    }
    validateDisplayName(displayName) {
        ...
        if (displayName.length > DISPLAY_NAME_MAX_LENGTH) {
            throw new common_1.BadRequestException('USER_DISPLAY_NAME_TOO_LONG');
        }
    }
    validateBio(bio) {
        if (!bio) {
        ...
        if (bio.length > BIO_MAX_LENGTH) {
            throw new common_1.BadRequestException('USER_BIO_TOO_LONG');
        }
    }
    validateAvatar(avatar) {
        ...
        if (avatar.length > AVATAR_MAX_LENGTH) {
            throw new common_1.BadRequestException('USER_AVATAR_TOO_LONG');
        }
        if (!/^https?:\/\//i.test(avatar)) {
            throw new common_1.BadRequestException('USER_AVATAR_INVALID_URL');
        }
    }
    validateAdminNote(note) {
        if (!note) {
            return;
        }
        if (note.length > ADMIN_NOTE_MAX_LENGTH) {
            throw new common_1.BadRequestException('USER_ADMIN_NOTE_TOO_LONG');
        }
    }
    async ensureUsernameAvailable(username, excludeUserId) {
        ...
        if (existing) {
            throw new common_1.ConflictException('USER_USERNAME_TAKEN');
        }
    }
    async ensureEmailAvailable(email, excludeUserId) {
        ...
        if (existing) {
            throw new common_1.ConflictException('USER_EMAIL_TAKEN');
        }
    }
    buildAdminQueryWhere(input) {
        const and = [];
        if (input.keyword) {
            and.push({
                OR: [
        ...
        if (input.status === 'BANNED') {
            and.push({
                status: prisma_client_1.UserStatus.DISABLED,
            });
        }
        if (input.status === 'PENDING') {
            if (!input.mailVerificationEnforced) {
                ...
                and.push({
                    status: prisma_client_1.UserStatus.ACTIVE,
                    passwordHash: {
                        not: null,
                    },
                    email: {
    ...
    resolveAuthProviders(row) {
        const providers = new Set();
        if (row.passwordHash) {
            providers.add('LOCAL');
        }
        const hasGithub = Boolean(row.githubId) ||
            row.oauthAccounts.some((item) => item.provider === prisma_client_1.OAuthProvider.GITHUB && item.isActive !== false);
        if (hasGithub) {
            providers.add('GITHUB');
        }
        const hasGoogle = row.oauthAccounts.some((item) => item.provider === prisma_client_1.OAuthProvider.GOOGLE && item.isActive !== false);
        if (hasGoogle) {
            providers.add('GOOGLE');
        }
        const orderedProviders = ['LOCAL', 'GITHUB', 'GOOGLE'];
        return orderedProviders.filter((item) => providers.has(item));
    }
    resolveAuthProvider(row) {
    ...
    resolveStatusView(row, mailVerificationEnforced) {
        return this.mailVerificationPolicyService.resolveAccountStatus(row, mailVerificationEnforced);
    }
    mapSelfProfile(row, mailVerificationEnforced) {
        const authProviders = this.resolveAuthProviders(row);
        const authProvider = this.resolveAuthProvider(row);
        const verificationRequiredNow = this.mailVerificationPolicyService.shouldRequireEmailVerificationNow(row, mailVerificationEnforced);
        return {
            id: row.id,
    ...
    mapAdminUser(row, mailVerificationEnforced) {
        const authProviders = this.resolveAuthProviders(row);
        const authProvider = this.resolveAuthProvider(row);
        const verificationRequiredNow = this.mailVerificationPolicyService.shouldRequireEmailVerificationNow(row, mailVerificationEnforced);
        return {
    ...
    async ensureLastAdminProtection(current, next) {
        const currentlyActiveAdmin = current.role === prisma_client_1.Role.ADMIN && current.status === prisma_client_1.UserStatus.ACTIVE;
        const keepsActiveAdmin = next.nextRole === prisma_client_1.Role.ADMIN && next.nextStatus === prisma_client_1.UserStatus.ACTIVE;
        ...
        if (anotherAdminCount <= 0) {
            throw new common_1.BadRequestException('USER_LAST_ADMIN_PROTECTED');
        }
    }
    async buildGithubUsername(rawUsername) {
    ...
    async createAdminActionLog(operatorId, targetUserId, action, detail) {
    ...
    generateTemporaryPassword() {
        const raw = (0, node_crypto_1.randomBytes)(18)
            .toString('base64url')
            .replace(/[^a-zA-Z0-9]/g, '');
