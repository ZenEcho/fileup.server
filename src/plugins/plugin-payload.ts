...
function validateInput(value: unknown, index: number) {
  if (!isRecord(value)) {
    throw new BadRequestException(`content.inputs[${index}] must be an object`);
  }
  ...
  if (value.options !== undefined && !Array.isArray(value.options)) {
    throw new BadRequestException(
      `content.inputs[${index}].options must be an array`,
    );
  }
  ...
  validateCondition(
    value.disabledWhen,
    `content.inputs[${index}].disabledWhen`,
  );
...
function normalizeContent(
  ...
  normalized: Omit<NormalizedCreatePluginPayload, 'content'>,
) {
  ...
  if (!Array.isArray(content.inputs)) {
    throw new BadRequestException('content.inputs must be an array');
  }
